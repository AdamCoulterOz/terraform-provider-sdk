...
public sealed class AttributePath(IEnumerable<TerraformAttributePathStep> steps) : IEquatable<AttributePath>
{
    ...
    public static AttributePath Root(string attributeName) =>
        new([TerraformAttributePathStep.Attribute(attributeName)]);

    public AttributePath WithAttribute(string attributeName) =>
        new(_steps.Concat([TerraformAttributePathStep.Attribute(attributeName)]));

    public AttributePath WithElementKey(string key) =>
        new(_steps.Concat([TerraformAttributePathStep.Element(key)]));

    public AttributePath WithElementIndex(long index) =>
    ...
    public override bool Equals(object? obj) => Equals(obj as AttributePath);
