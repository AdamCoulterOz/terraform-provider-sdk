...
public static class TerraformProviderHost
{
    public static Task<int> RunAsync<TConfig, TProviderState>(
        Provider<TConfig, TProviderState> provider,
        string[] args,
        CancellationToken cancellationToken = default)
        where TConfig : new() =>
        RunAsync(provider.ToInternalProvider(), args, cancellationToken);

    internal static async Task<int> RunAsync(IProvider provider, string[] args, CancellationToken cancellationToken = default)
    {
        if (!IsPluginLaunch())
        {
