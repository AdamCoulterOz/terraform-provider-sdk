...
internal static class ModelConventions
{
    ...
    {
        var attributeName = member.GetCustomAttribute<TFAttributeAttribute>(inherit: true)?.Name;
        ...
        var nestedBlockName = member.GetCustomAttribute<NestedBlockAttribute>(inherit: true)?.TypeName;
    ...
    public static bool TryGetValueType(Type type, out Type wrappedType)
    {
    ...
    {
        if (member.GetCustomAttribute<TFAttributeAttribute>(inherit: true) is { Computed: true })
        {
            return false;
        }
    ...
    private static bool ShouldIncludeMember(MemberInfo member) =>
        member.GetCustomAttribute<TFAttributeAttribute>(inherit: true) is not null ||
        member.GetCustomAttribute<NestedBlockAttribute>(inherit: true) is not null ||
        member switch
        {
