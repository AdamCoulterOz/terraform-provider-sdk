    ...
    IHostApplicationLifetime applicationLifetime) : ProviderServiceBase
{
    ...
    public override Task<GetProviderSchema.Types.Response> GetProviderSchema(GetProviderSchema.Types.Request request, ServerCallContext context)
    {
        var response = new GetProviderSchema.Types.Response
        {
            ...
            Provider = TerraformSchemaMapper.ToProtocolSchema(provider.ProviderSchema),
        };

        if (provider.ProviderMetaSchema is not null)
        {
            response.ProviderMeta = TerraformSchemaMapper.ToProtocolSchema(provider.ProviderMetaSchema);
        }

        foreach (var resource in provider.Resources.OrderBy(static pair => pair.Key, StringComparer.Ordinal))
        {
            response.ResourceSchemas[resource.Key] = TerraformSchemaMapper.ToProtocolSchema(resource.Value.Schema);
        }

        foreach (var dataSource in provider.DataSources.OrderBy(static pair => pair.Key, StringComparer.Ordinal))
        {
            response.DataSourceSchemas[dataSource.Key] = TerraformSchemaMapper.ToProtocolSchema(dataSource.Value.Schema);
        }
    ...
    public override Task<GetResourceIdentitySchemas.Types.Response> GetResourceIdentitySchemas(GetResourceIdentitySchemas.Types.Request request, ServerCallContext context) =>
        Task.FromResult(new GetResourceIdentitySchemas.Types.Response());
    ...
    {
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, provider.ProviderSchema.Block.ValueType());
        var result = await provider.ValidateConfigAsync(new TerraformProviderValidateRequest(config), context.CancellationToken).ConfigureAwait(false);
    ...
    public override async Task<ConfigureProvider.Types.Response> ConfigureProvider(ConfigureProvider.Types.Request request, ServerCallContext context)
    {
        ...
        var result = await provider.ConfigureAsync(
            new TerraformProviderConfigureRequest(
                config,
    ...
    public override Task<UpgradeResourceState.Types.Response> UpgradeResourceState(UpgradeResourceState.Types.Request request, ServerCallContext context)
    {
        ...
        {
            var upgradedState = TerraformDynamicValueSerializer.DecodeJsonValue(request.RawState.Json.Span, stateType);
            response.UpgradedState = TerraformDynamicValueSerializer.EncodeDynamicValue(upgradedState, stateType);
            return Task.FromResult(response);
        }
        ...
        response.Diagnostics.Add(ToProtocolDiagnostic(TerraformDiagnostic.Error(
    ...
    {
        var resource = GetResource(request.TypeName);
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, resource.Schema.Block.ValueType());
        var result = await resource.ValidateConfigAsync(new TerraformResourceValidateRequest(config), context.CancellationToken).ConfigureAwait(false);
    ...
    public override async Task<ValidateDataResourceConfig.Types.Response> ValidateDataResourceConfig(ValidateDataResourceConfig.Types.Request request, ServerCallContext context)
    {
        ...
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, dataSource.Schema.Block.ValueType());
        var result = await dataSource.ValidateConfigAsync(new TerraformDataSourceValidateRequest(config), context.CancellationToken).ConfigureAwait(false);
    ...
    public override async Task<ReadResource.Types.Response> ReadResource(ReadResource.Types.Request request, ServerCallContext context)
    {
        var resource = GetResource(request.TypeName);
        var stateType = resource.Schema.Block.ValueType();
        var currentState = TerraformDynamicValueSerializer.DecodeDynamicValue(request.CurrentState, stateType);
        var result = await resource.ReadAsync(
            new TerraformResourceReadRequest(currentState, request.Private.ToByteArray(), sessionState.ProviderState),
            context.CancellationToken).ConfigureAwait(false);

        return new ReadResource.Types.Response
        {
            NewState = TerraformDynamicValueSerializer.EncodeDynamicValue(result.NewState, stateType),
    ...
    public override async Task<PlanResourceChange.Types.Response> PlanResourceChange(PlanResourceChange.Types.Request request, ServerCallContext context)
    {
        var resource = GetResource(request.TypeName);
        var stateType = resource.Schema.Block.ValueType();
        var priorState = TerraformDynamicValueSerializer.DecodeDynamicValue(request.PriorState, stateType);
        var proposedNewState = TerraformDynamicValueSerializer.DecodeDynamicValue(request.ProposedNewState, stateType);
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, stateType);
        var result = await resource.PlanAsync(
            new TerraformResourcePlanRequest(priorState, proposedNewState, config, request.PriorPrivate.ToByteArray(), sessionState.ProviderState),
            context.CancellationToken).ConfigureAwait(false);

        var response = new PlanResourceChange.Types.Response
        {
            PlannedState = TerraformDynamicValueSerializer.EncodeDynamicValue(result.PlannedState, stateType),
    ...
    public override async Task<ApplyResourceChange.Types.Response> ApplyResourceChange(ApplyResourceChange.Types.Request request, ServerCallContext context)
    {
        ...
        var stateType = resource.Schema.Block.ValueType();
        var priorState = TerraformDynamicValueSerializer.DecodeDynamicValue(request.PriorState, stateType);
        var plannedState = TerraformDynamicValueSerializer.DecodeDynamicValue(request.PlannedState, stateType);
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, stateType);
        var result = await resource.ApplyAsync(
            new TerraformResourceApplyRequest(priorState, plannedState, config, request.PlannedPrivate.ToByteArray(), sessionState.ProviderState),
            context.CancellationToken).ConfigureAwait(false);
    ...
    public override async Task<ImportResourceState.Types.Response> ImportResourceState(ImportResourceState.Types.Request request, ServerCallContext context)
    {
        ...
        var result = await resource.ImportAsync(
            new TerraformResourceImportRequest(request.TypeName, request.Id, sessionState.ProviderState),
            context.CancellationToken).ConfigureAwait(false);
    ...
    public override async Task<ReadDataSource.Types.Response> ReadDataSource(ReadDataSource.Types.Request request, ServerCallContext context)
    {
        ...
        var config = TerraformDynamicValueSerializer.DecodeDynamicValue(request.Config, stateType);
        var result = await dataSource.ReadAsync(new TerraformDataSourceReadRequest(config, sessionState.ProviderState), context.CancellationToken).ConfigureAwait(false);

        return new ReadDataSource.Types.Response
        {
            State = TerraformDynamicValueSerializer.EncodeDynamicValue(result.NewState, stateType),
            Diagnostics = { result.Diagnostics?.Select(ToProtocolDiagnostic) ?? [] },
        };
    }
    ...
    public override Task<ValidateListResourceConfig.Types.Response> ValidateListResourceConfig(ValidateListResourceConfig.Types.Request request, ServerCallContext context) =>
        Task.FromResult(new ValidateListResourceConfig.Types.Response());
    ...
    public override Task ListResource(ListResource.Types.Request request, IServerStreamWriter<ListResource.Types.Event> responseStream, ServerCallContext context) =>
        Task.FromException(new RpcException(new Status(StatusCode.Unimplemented, "List resources are not supported.")));
    ...
    private ITerraformResource GetResource(string typeName) =>
    ...
    private ITerraformDataSource GetDataSource(string typeName) =>
    ...
    private static Diagnostic ToProtocolDiagnostic(TerraformDiagnostic diagnostic) =>
        new()
        {
            Severity = diagnostic.Severity == TerraformDiagnosticSeverity.Warning
                ? Diagnostic.Types.Severity.Warning
                : Diagnostic.Types.Severity.Error,
            Summary = diagnostic.Summary,
            Detail = diagnostic.Detail,
            Attribute = diagnostic.Attribute is null ? null : ToProtocolAttributePath(diagnostic.Attribute),
        };

    private static AttributePath ToProtocolAttributePath(TerraformAttributePath path)
    {
        var attributePath = new AttributePath();

        foreach (var step in path.Steps)
        {
            var protocolStep = new AttributePath.Types.Step();
